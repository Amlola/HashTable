...
Распределение хеш-функций будут визуализированы с помощью гистограмм. По горизонтали номер списка в таблице, А по вертикали количество элементов в соответствующем списке. А критерием сравнения функций будет являться ```дисперсия```, так как она определяет насколько равномерно значения распределяются по спискам. Поэтому чем меньше ее значение, тем лучше функция.   
> [!NOTE]
...
Мы видим, что в этом случае уже нет никакого равномерного распределения. Это связано с огранниченостью длины английских слов. Так как коды маленьких английских букв лежат в диапазоне от 97 до 122, то при большом размере хеш-таблицы в последних ячейках уже не будет находиться элементов.  
...
Время выполнения такой программы в количестве выполненных инструкций:  
...
Почему же оптимизация данной функции является проблемой?  
...
### Оптимизация оператора остатка от деления
